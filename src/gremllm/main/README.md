...
`acp/prompt`. The ACP working directory is the active document's parent
directory; prompts attach the document path itself. Session runtime behavior
lives in `main.effects.acp`, while prompt content construction lives in
`main.actions.acp`. The JS transport bridge
