...
(defn send-message-from-ipc
  ...
  (let [workspace-dir    (state/get-workspace-dir state)
    ...
    (if has-attachments?
      ...
      [[:chat.actions/send-message-with-attachments workspace-dir attachment-paths messages model api-key reasoning]]
...
(defn send-message-with-attachments
  [_state workspace-dir file-paths messages model api-key reasoning]
  [[:attachment.effects/prepare-for-send workspace-dir file-paths messages model api-key reasoning]])
...
(defn attach-and-send
  ...
  [_state loaded-pairs messages model api-key reasoning]
